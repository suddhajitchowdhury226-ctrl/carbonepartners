...
const SmsfAdministrationPage = ({ onBack }) => {
  useEffect(() => { window.scrollTo({ top: 0, behavior: 'smooth' }); }, []);
  return (
    <div>
      <PageHero onBack={onBack} title="SMSF Administration" subtitle="Managing Your Fund" />
      <ContentSection content={
        <>
          ...
          <ul style={{ display: 'flex', gap: 16, marginBottom: 32, flexWrap: 'wrap', listStyle: 'none', padding: 0 }}>
            {['Australian Taxation Office (ATO)', 'APRA', 'ASIC'].map((auth, i) => (
              <li key={i} style={{ background: '#143c82', color: '#fff', padding: '8px 16px', borderRadius: 20, fontSize: 13, fontWeight: 600, letterSpacing: 0.5 }}>{auth}</li>
            ))}
          </ul>
          
          <p style={{ marginBottom: 40, lineHeight: 1.8 }}>
          ...
          <div style={{ borderTop: '4px solid #c8a96e', background: '#f8f9fa', padding: '40px', borderRadius: '0 0 12px 12px' }}>
...
export default SmsfAdministrationPage;
