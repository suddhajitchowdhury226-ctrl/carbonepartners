...
const PayrollManagementPage = ({ onBack }) => {
  useEffect(() => { window.scrollTo({ top: 0, behavior: 'smooth' }); }, []);
  return (
    <div>
      <PageHero onBack={onBack} title="Payroll Management" subtitle="Local STP Compliance" />
      <ContentSection content={
        <>
          <p style={{ marginBottom: 32, fontSize: 17, color: '#080f1e', fontWeight: 500, lineHeight: 1.8 }}>
            With the introduction of Single Touch Payroll (STP) compliance, managing salaries within your business can become stressful. 
          </p>
          <div style={{ background: '#f8f9fa', padding: '36px', borderRadius: 12, borderLeft: '4px solid #143c82', marginBottom: 36 }}>
            <p style={{ margin: 0, lineHeight: 1.8, color: '#464650' }}>
              At Carbone Partners we can offer a full payroll management service, and have qualified bookkeepers that can prepare all your payroll and PAYG documentation, accurately and on-time, giving you more time to concentrate on managing your team.
            </p>
          </div>
          
          <h2 style={{ fontSize: 20, fontWeight: 800, color: '#143c82', marginBottom: 20 }}>Our Payroll management services include:</h2>
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
            {[
              'Payroll set ups',
              'Payroll Processing & Reporting (onsite or remote)',
              'IAS',
              'Superannuation',
              'End of Financial Year Payment Summaries and Lodgement to ATO'
            ].map((item, i) => (
              <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#fff', padding: '16px 20px', borderRadius: 8, border: '1px solid rgba(0,0,0,0.08)' }}>
                ...
                <span style={{ fontSize: 14.5, color: '#464650', fontWeight: 500 }}>{item}</span>
              </li>
            ))}
          </ul>
        </>
