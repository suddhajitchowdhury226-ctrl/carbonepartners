...
const PageHero = ({ onBack, title, subtitle }) => (
  <section style={{
    ...
    background: 'linear-gradient(135deg, #080f1e 0%, #0c1a36 55%, #0f2347 100%)',
    display: 'flex', alignItems: 'center',
    overflow: 'hidden', paddingTop: 90, paddingBottom: 60
  }}>
    <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 5, background: 'linear-gradient(180deg,#c8a96e,#b8924e)' }} />
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', backgroundImage: `linear-gradient(rgba(255,255,255,0.022) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.022) 1px,transparent 1px)`, backgroundSize: '60px 60px' }} />
    <div style={{ position: 'absolute', right: -60, top: -80, width: 480, height: 480, borderRadius: '50%', background: 'radial-gradient(circle,rgba(20,60,130,0.22) 0%,transparent 70%)', pointerEvents: 'none' }} />
...
const VehicleEquipmentPage = ({ onBack }) => {
  useEffect(() => { window.scrollTo({ top: 0, behavior: 'smooth' }); }, []);
  return (
    <div>
      <PageHero onBack={onBack} title="Vehicle & Equipment Finance" subtitle="Funding Your Assets" />
      <ContentSection content={
        <>
          <p style={{ marginBottom: 20, fontSize: 17, color: '#080f1e', fontWeight: 500 }}>
            Needing commercial vehicles or machinery? Asset finance can help you get there.
          </p>
          <p style={{ marginBottom: 40 }}>
            Looking to get a new a motor vehicle, fleet of trucks, or the latest plant and equipment to take your business to the next level. Call us today and let us take the stress out of funding your next asset purchase.
          </p>
          
          <h2 style={{ fontSize: 22, fontWeight: 800, color: '#143c82', marginBottom: 24 }}>We can provide finance for a wide range of business equipment including but not limited to:</h2>
          
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
            {[
              'Passenger vehicles',
              'Commercial vehicles',
              ...
              <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#f8f9fa', padding: '20px 24px', borderRadius: 8, border: '1px solid rgba(0,0,0,0.04)' }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(20,60,130,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <CheckCircle2 size={16} color="#143c82" />
                </div>
                <span style={{ fontSize: 15, color: '#464650', fontWeight: 500 }}>{item}</span>
              </li>
            ))}
          </ul>
        </>
