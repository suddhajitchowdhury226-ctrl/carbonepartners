  ...
  <section style={{
    ...
    <div className="container" style={{ position: 'relative', zIndex: 3, padding: '60px 32px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 32 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)' }}>Superannuation</span>
        <ChevronRight size={11} color="rgba(255,255,255,0.3)" />
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 48, flexWrap: 'wrap' }}>
          <div style={{ maxWidth: 580 }}>
            ...
            <p style={{ fontSize: 14.5, lineHeight: 1.85, color: 'rgba(255,255,255,0.55)', maxWidth: 480 }}>
              Let’s discuss how we can help you build and protect your retirement wealth with tailored Self-Managed Superannuation strategies.
            </p>
          </div>
...
const SmsfStrategyPage = ({ onBack }) => {
  useEffect(() => { window.scrollTo({ top: 0, behavior: 'smooth' }); }, []);
  return (
    <div>
      <PageHero onBack={onBack} title="SMSF Strategy" subtitle="Maximizing Your Fund" />
      <ContentSection content={
        <>
          ...
          <div style={{ background: '#f8f9fa', padding: '40px', borderRadius: 12, borderLeft: '4px solid #143c82' }}>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: '#143c82', marginBottom: 24 }}>The benefits of running your own fund include:</h2>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 20 }}>
              {[
                { title: 'Flexibility & Control', desc: 'SMSFs allow multiple members to run a mixture of accumulation and pension accounts. Trustees have complete control over what happens with the fund.' },
                { title: 'Investment Choice', desc: 'SMSFs provide greater investment options than any other super fund.' },
                { title: 'Taxation Strategies', desc: 'Concessional tax environment where earnings are taxed at 15% in Accumulation Phase and tax-free in Pension Phase. We can help trustees grow their super savings and reduce tax payments.' },
                { title: 'Fund Costs', desc: 'Depending on the size of your fund, can be cheaper to run than a large public fund.' },
                { title: 'Unique Investment Strategies', desc: 'Including gearing in superannuation.' },
                { title: 'Creditor Protection', desc: 'Assets are normally protected by creditors even in the event of bankruptcy.' },
                { title: 'Consolidate Superannuation Assets', desc: 'A SMSF allows up to three other members to consolidate super accounts. A larger fund can increase the overall investment opportunities whilst reducing the overall fees.' },
                { title: 'Creation of a Family Legacy', desc: 'Pass wealth down securely.' }
              ].map((item, i) => (
...
export default SmsfStrategyPage;
