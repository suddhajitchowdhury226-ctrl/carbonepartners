...
const PersonalLoansPage = ({ onBack }) => {
  useEffect(() => { window.scrollTo({ top: 0, behavior: 'smooth' }); }, []);
  return (
    <div>
      <PageHero onBack={onBack} title="Personal Loans" subtitle="Funding Your Dream" />
      <ContentSection content={
        <>
          <p style={{ marginBottom: 40, fontSize: 17, color: '#080f1e', fontWeight: 500 }}>
            Everybody has a dream. And whilst they can vary widely from person to person, one thing remains constant – the need to fund you dream.
          </p>
          
          <div style={{ background: '#f8f9fa', padding: '40px', borderRadius: 12, borderLeft: '4px solid #143c82', marginBottom: 40 }}>
            <h2 style={{ fontSize: 20, fontWeight: 800, color: '#080f1e', marginBottom: 20 }}>Your dream may be:</h2>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14 }}>
              {[
                ...
                <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: 'rgba(20,60,130,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <p style={{ marginBottom: 20, lineHeight: 1.8 }}>
            We will ensure that whatever the changes may be, the best possible finance solution is tailored to you best meet those changes. Talk to us today to compare hundreds of personal loan options.
          </p>
        </>
      } />
      <CTABanner />
